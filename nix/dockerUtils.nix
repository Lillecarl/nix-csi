pkgs: {
  ...
  nonRootShadowSetup =
    { users, shell ? "${pkgs.runtimeShell}" }:
    let
      passwdEntries =
        map (user:
          "${user.name}:x:${toString user.id}:${toString user.id}::/home/${user.name}:${if user ? shell then user.shell else shell}"
        ) users;
      ...
      groupEntries =
        map (user: "${user.name}:x:${toString user.id}:${toString user.name}") users;
